...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GKUITabListWidgetBase)

void UGKUITabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UGKUITabListWidgetBase::NativeConstruct()
{
...
void UGKUITabListWidgetBase::NativeDestruct()
{
	for (FGKUITabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UGKUITabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FGKUITabDescriptor& OutTabInfo)
{
	const FGKUITabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FGKUITabDescriptor& TabInfo) -> bool
	{
...
void UGKUITabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FGKUITabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UGKUITabListWidgetBase::RegisterDynamicTab(const FGKUITabDescriptor& TabDescriptor)
{
...
void UGKUITabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FGKUITabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UGKUITabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UGKUITabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FGKUITabDescriptor* TabInfoPtr = nullptr;
	
	FGKUITabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UGKUITabButtonInterface::StaticClass()))
	{
		...
		{
			IGKUITabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UGKUITabListWidgetBase::IsFirstTabActive() const
{
...
bool UGKUITabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UGKUITabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UGKUITabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UGKUITabListWidgetBase::SetupTabs()
{
	for (FGKUITabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
